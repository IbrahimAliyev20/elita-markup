...
// Tip tərifini burada da yeniləyirik
interface InformationItem {
  ...
  gallery_link?: string | null; // Yeni sahə
}

// Prop-lardan galleryCategories tamamilə çıxarılır
...
function AboutSection({ information }: AboutSectionProps) {
  const [expandedItems, setExpandedItems] = useState<boolean[]>(
    new Array(information?.length || 0).fill(false)
  );
  ...
  useEffect(() => {
    // information mövcud deyilsə, funksiyanı dayandır
  ...
  // Əgər information gəlməyibsə heçnə göstərmə (təhlükəsizlik üçün)
  if (!information) {
    return null;
  }
        ...
        <div className="flex gap-2 overflow-x-auto whitespace-nowrap scrollbar-hide md:flex-wrap md:overflow-visible md:whitespace-normal">
          {uniqueTitles.map((title) => (
            <button
              ...
              className="px-4 py-2 rounded-md border font-medium transition-colors duration-200 bg-white text-[#5A635C] border-[#5A635C] hover:bg-[#5A635C] hover:text-white flex-shrink-0"
            >
              {title}
            </button>
          ))}
        ...
        >
          <div className="w-full h-auto grid grid-cols-1 xl:grid-cols-3 md:gap-[100px] mb-10 xl:mb-16">
            <div className="col-span-1 flex flex-col">
              <div className="flex gap-3 items-center">
                ...
                <svg className="flex-1 h-[1px]" preserveAspectRatio="none" viewBox="0 0 100 1">
                  <line x1="0" y1="0.5" x2="100" y2="0.5" stroke="currentColor" strokeWidth="1" className="text-elements" />
                </svg>
              </div>
            ...
            <div className="col-span-2 flex flex-col mt-8 xl:mt-0">
              <div className="w-full">
                <div className="hidden md:flex items-center h-[23px] mt-[3px]">
                  <svg className="w-full h-[1px]" preserveAspectRatio="none" viewBox="0 0 100 1">
                    <line x1="0" y1="0.5" x2="100" y2="0.5" stroke="currentColor" strokeWidth="1" className="text-elements" />
                  </svg>
                </div>
          ...
          <div className="w-full grid grid-cols-1 xl:grid-cols-3 md:gap-[100px] mb-10 xl:mb-16 ">
            <div className="hidden xl:flex col-span-1 flex-col gap-10">
              <div className="w-full">
                <Image width={500} height={500} src={item.image_1} alt={`${item.title} - Image 1`} className="w-full h-[300px] object-cover"/>
              </div>
              <div className="w-full">
                <Image width={500} height={500} src={item.image_2} alt={`${item.title} - Image 2`} className="w-full h-[300px] object-cover"/>
              </div>
            </div>
            <div className="w-full h-auto col-span-2 ">
              <Image width={500} height={500} src={item.image_3} alt={`${item.title} - Main Image`} className="w-full h-[200px] md:h-[640px] object-cover"/>
            </div>
          </div>
        </div>
      ))}
