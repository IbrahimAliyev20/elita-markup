// src/components/DesignPage/AboutSection.tsx
...
function AboutSection({ information, servicesContainerRef }: AboutSectionProps) {
  ...
  const baseTitles = Array.from(
    new Set(information?.map((item) => item.title) || [])
  );

  // 2. Düymə sırasını dəyişin: "Xidmətlər" sona əlavə olunsun
  const uniqueTitles =
    pathname === "/dizayn" ? [...baseTitles, t("services")] : baseTitles;
  ...
  const handleScrollToSection = (title: string) => {
    ...
    if (title === t("services")) {
      if (servicesContainerRef.current) {
        const elementPosition = servicesContainerRef.current.offsetTop;
